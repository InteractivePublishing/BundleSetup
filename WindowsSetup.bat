...
call %~dp0\Setup\util\var_line_parser %~dp0\setup_vars.txt
...
set "BaseInstallPath=C:\InteractivePublishing"
REM echo "i:%InstallPath%"
...
@REM Extract data
echo.>%DataUninst% 
for /f "usebackq delims=|" %%f in (`dir /b "%~dp0*.zip"`) do ( 
  echo Extract - %%f
...
if exist %idx_LibConf% (
    ...
    call %~dp0\Setup\util\var_line_parser %DataPath%\%LibIndex%\libvars.txt
...
if exist %idx_LibConf% (
  call %~dp0\Setup\util\remove_blanks %idx_LibConf%
  @REM del %idx_LibConf% 
  ...
  @REM echo call %~dp0Setup\util\var_line_parser %DataPath%\%LibIndex%\libvars.txt
  call %~dp0\Setup\util\var_line_parser %DataPath%\%LibIndex%\libvars.txt
...
echo %~dp0Setup\util\shortcut.bat %InstallPath%\AtlasViewer.exe %InstallPath% InteractivePublishingDataViewer "--ndLibrary %DataPathAlt%/%LibIndex%" >>%LogPath%
if not exist %StartShortcut%.lnk (
  echo "Making lib link to %LibItemString%%LibName%"
  echo continue in 
  call %~dp0Setup\util\shortcut.bat %InstallPath%\AtlasViewer.exe %InstallPath% InteractivePublishingDataViewer "--ndLibrary %DataPathAlt%/%LibIndex%"
